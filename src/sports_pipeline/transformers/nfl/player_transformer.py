...
class NflPlayerTransformer(BaseTransformer):
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["team_canonical"] = df["team"].apply(
            lambda x: self.normalizer.normalize_team(str(x), "football") if pd.notna(x) else x
        )
        ...
        receiving_tds = (
            pd.to_numeric(df.get("receiving_tds", 0), errors="coerce").fillna(0).astype(int)
        )
        ...
        silver = pd.DataFrame(
            {
                ...
                # Passing
                "completions": pd.to_numeric(df.get("completions", 0), errors="coerce")
                .fillna(0)
                .astype(int),
                "passing_attempts": pd.to_numeric(df.get("passing_attempts", 0), errors="coerce")
                .fillna(0)
                .astype(int),
                "passing_yards": passing_yards,
                "passing_tds": passing_tds,
                "interceptions": pd.to_numeric(df.get("interceptions", 0), errors="coerce")
                .fillna(0)
                .astype(int),
                "sacks": pd.to_numeric(df.get("sacks", 0), errors="coerce").fillna(0).astype(int),
                # Rushing
                "carries": pd.to_numeric(df.get("carries", 0), errors="coerce")
                .fillna(0)
                .astype(int),
                "rushing_yards": rushing_yards,
                "rushing_tds": rushing_tds,
                "rushing_fumbles": pd.to_numeric(df.get("rushing_fumbles", 0), errors="coerce")
                .fillna(0)
                .astype(int),
                # Receiving
                "receptions": pd.to_numeric(df.get("receptions", 0), errors="coerce")
                .fillna(0)
                .astype(int),
                ...
                "fantasy_points": pd.to_numeric(
                    df.get("fantasy_points", 0), errors="coerce"
                ).fillna(0.0),
                # Derived
